...
# the following should be all the translations of "Template" in our po files
...
class Form():
    ...
    """
    def __init__(self, definition_files=definition_files):
        self.__dates = {}
        self.__headings = {}
        self.__sections = {}
        self.__columns = {}
        ...
        for file_name in definition_files:
            full_path = os.path.join(os.path.dirname(__file__), "Forms", file_name)
    ...
    def __load_definitions(self, definition_file):
        ...
        # iterate through form defs
        for form in top[0].getElementsByTagName('form'):

            # get form id, title, type
            ...
            """
            Get details about form- ex. Sheet No. for Census
            """
            ...
            """
            Get details about individuals appearing in form
            """
            ...
            for section in sections:
                ...
                # Get grouping labels for sections (ex. Children)
                if 'title' in section.attributes:
                    title = section.attributes['title'].value
                else:
                    title = ''

                # Get role assigned to section
                role = section.attributes['role'].value

                # Type is either multi, person, or family
...
def get_template_id(source):
    """
    Return the template id attached to the given source.
    """
    for attr in source.get_attribute_list():
        if str(attr.get_type()) in INTL_TEMPLATE:
            return attr.get_value()
    return None
