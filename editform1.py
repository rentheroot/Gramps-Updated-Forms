...
from form import (get_form_id, get_form_date, get_form_type, get_form_headings,
                  ...
                  get_section_columns, get_form_citation, get_template_id)
from entrygrid import EntryGrid
from eventmanager import ManageEvents
...
class EditForm(ManagedWindow):
    ...
    def __init__(self, dbstate, uistate, track, event, citation, callback):
        ...
        # Events from templates management
        self.EventManager = ManageEvents()
    ...
    def __populate_gui(self, event):
        ...
        template_id = get_template_id(source)
...
class DetailsTab(GrampsTab):
    ...
    """
    def __init__(self, dbstate, uistate, track, event, citation, form_id, template_id):
        self.db = dbstate.db
        self.event = event
        self.citation = citation
...
class MultiSection(Gtk.Box):
    ...
    def populate_gui(self, event):
        ...
        # Find all people attached to master census event
        ...
        # Fill out form table using event reference attributes
    ...
    def save(self, trans):
        ...
        for order, row in enumerate(self.model):
            ...
            event_creation_rules = self.EventManager.extract_rules(event_creation_rules)

            # Get events connected to person
            connected_personal_events = self.EventManager.load_connected_events(master_event_ref=event_ref,
                                                                                db=self.db,
                                                                                person_handle=person)
            event_to_column = self.EventManager.map_event_to_column(self.db,
            ...
            for offset, name in enumerate(self.columns):
                value = row[offset + 1]
                heading_to_value[offset+1] = value

                headings_to_col_num[name] = offset + 1

            self.EventManager.update_events(db=self.db,
                                            event_to_col=event_to_column,
                                            connected_personal_events=connected_personal_events,
                                            heading_to_value=heading_to_value,
                                            trans=trans)
            
            self.EventManager.create_new_events(event_creation_rules,
                                                connected_personal_events,
                                                headings_to_col_num,
                                                event_to_column)
...
# Each attribute tied to form value
